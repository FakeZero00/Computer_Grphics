...
void Default_Scene::LoadScene(AppContext& ctx) {
	...
	Object* borderHorizontal = Instantiate(ctx, "BorderHorizontal");
	borderHorizontal->AddComponent<Spline>(HorizontalCP, vec4{ 0.0f, 1.0f, 0.0f, 1.0f });

	Object* borderVertical = Instantiate(ctx, "BorderVertical");
	borderVertical->AddComponent<Spline>(VerticalCP, vec4{ 0.0f, 1.0f, 0.0f, 1.0f });

	Object* shapeGeneratorObj = Instantiate(ctx, "ShapeGenerator");
	shapeGeneratorObj->AddComponent<ShapeGenerator>(isoPolyMesh, isoPolyCP);
}