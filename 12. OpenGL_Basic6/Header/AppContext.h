...
struct AppContext {
	...
	vector<BoxCollider2D*> CollisionObjects;				//충돌 감지를 위한 오브젝트 벡터
	vector<BoxCollider2D*> pendingCollisionObjects;			//새로 생성된 충돌 감지 오브젝트를 임시로 저장하는 벡터
	vector<BoxCollider2D*> pendingDestroyCollisionObjects;	//제거할 충돌 감지 오브젝트를 저장하는 벡터
};